# encoding: utf-8
require "acts_as_xapian/version"
...
module ActsAsXapian
    ...
    def ActsAsXapian.prepare_environment
      ...
      raise "Set Rails environment, so acts_as_xapian can find the right Xapian database" if not environment

      # check for a config file
      config_file = File.join(Rails.root, "config", "xapian.yml")
      ...
      if config['base_db_path']
        db_parent_path = File.join(Rails.root, config['base_db_path'])
      else
